import json, os, subprocess, time, csv, math
...
def surrogate_objective(p: dict) -> float:
    """
    Placeholder objective until EM + thermal are plugged in.

    Higher is "better":
    - encourages larger effective area/volume for inductive coupling
    - penalizes wall thickness (mass)
    - rewards hollowness (reduced mass, potential eddy-current concentration)
    """
    ...
    # basic physical-ish proxies
    outer_vol = (1/3) * math.pi * (R**2) * L  # cone volume proxy
    inner_R = max(R - t, 0.0)
    inner_vol = (1/3) * math.pi * (inner_R**2) * min(d, L)

    mass_proxy = max(outer_vol - inner_vol, 1e-9)      # "material volume"
    coupling_proxy = (R * L)                           # bigger tip tends to couple more
    hollowness = inner_vol / max(outer_vol, 1e-9)

    thin_penalty = 0.0
    if t < 0.2:
        thin_penalty = (0.2 - t) * 50.0                # discourage too-thin walls

    # Score: maximize coupling, maximize hollowness, minimize mass
